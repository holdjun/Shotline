...
def _load_raw(path: Path, raw_params: dict[str, Any] | None = None) -> ImageData:
    """Load RAW as LINEAR float32 with optimized rawpy parameters.

    Parameters are read from raw_params dict (typically from
    [processor_params.raw_develop] in shotline.toml).
    """
    import rawpy
    ...
    fbdd = params.get("fbdd_noise_reduction", "Light")
