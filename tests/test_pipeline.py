...
def test_pipeline_with_stub_processors(sample_jpg: Path, tmp_path: Path):
    ...
    # raw_develop skipped (jpg not in supported_inputs), exposure_adjust and white_balance run
    assert "raw_develop" in result.skipped
    assert any(s["name"] == "exposure_adjust" for s in result.steps_run)
...
def test_pipeline_encoding_in_result(sample_jpg: Path, tmp_path: Path):
    ...
    out = tmp_path / "out.jpg"
    result = Pipeline(steps=["exposure_adjust", "white_balance"]).run(sample_jpg, out)
