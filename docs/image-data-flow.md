...
| `source_bit_depth` | 记录原始位深，用于保存时选择输出位深 |
...
| 格式 | 编码 | 加载方式 | 值域 | 位深 |
|------|------|----------|------|------|
| RAW (.cr2/.nef/.arw/...) | **LINEAR** | rawpy, `gamma=(1,1)` | [0, ~1.0+] | 16-bit |
...
photo.cr2
  │
  ▼  rawpy, gamma=(1,1), 16-bit output
  │
  ImageData(encoding=LINEAR, 值域 [0.0, ~1.0+])
  │  线性光数据，高光可能超过 1.0 — RAW 的核心优势
  │
  ▼  [10] raw_develop
  │  线性空间 EV 曝光补偿: data * 2^ev
  │  仍然 LINEAR
  │
  ▼  [15] tone_map
  │  filmic S 曲线压缩动态范围，然后 linear_to_srgb()
  │  ★ 变为 SRGB [0, 1] ★
  │
  ▼  [20+] denoise → horizon → white_balance → color_grade → auto_crop
  │  全部在 SRGB 空间工作
  │
...
photo.jpg
  ...
  │
  ▼  [10] raw_develop — 跳过 (supported_inputs=["raw"], JPG 不匹配)
  │
  ▼  [15] tone_map
  │  轻微 S 曲线: 微弱提亮暗部、压制高光
  │  仍然 SRGB
  │
  ▼  [20+] denoise → horizon → white_balance → ...
  │
  ▼  save_image → output.jpg
```
...
**`tone_map`（order 15）是两条路径的汇合点。** 经过 tone_map 后，无论输入是什么格式，数据都统一为 SRGB [0, 1]，后续处理器在同一个标准空间工作。
...
保持 LINEAR 编码到 tone_map 阶段：
- `raw_develop` 在线性空间做曝光补偿，`data * 2` 就是 +1EV，物理上精确
- `tone_map` 用 filmic 曲线优雅地压缩动态范围，高光自然滚降而非硬裁切
